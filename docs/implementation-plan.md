...
* **GPT-5.2 (`gpt-5.2-2025-12-11`, reasoning.effort `none`) Batch Client (OpenAI):**
    * Implement a class `OpenAIClient` with batch helpers (`build_batch_request`, `write_batch_requests`, `create_batch`, `parse_batch_output`).
    * **Logic:** Use batch-only submissions. No synchronous generation calls in the dataset pipeline. Do not auto-poll; output retrieval is manual/on-demand.
...
* **Category B (Facts):** Use Wikidata SPARQL + RestCountries cache, filter for single-word objects.
    * Relations: capital, currency, language, continent, occupation, demonym, birth_month (month name), national_sport (if available).
...
* **Category C (Common Sense):** Use OMCS sentences (Open Mind Common Sense) as the primary source; ConceptNet is expected to be down.
    * If OMCS/ConceptNet yields fewer than 1,000 candidates, fill the gap via GPT-5.2 (`gpt-5.2-2025-12-11`, reasoning.effort `none`) batch fallback.
    * Return structured JSON with subject, relation (one of 10 base relations), optional relation_label for diversity, target_word.
    * Build prompts using the 10 common-sense templates (C1–C10).
    * Enforce diversity (no duplicate subject/target, balance relation types across A–J variants).
...
* **Selection Rule:**
    ...
    * Hard 500 target: if bins/repetition caps are short, fill to 500 with next-highest S(p) prompts from remaining validated prompts; if fewer than 500 validated exist, log the shortfall (no regeneration).
* **Global Gating:**
...
* **Outputs:**
  ...
  - If needed for behavioral metrics, write samples mapping separately to `runs/detection_mapping.jsonl`.
...
* **Decision step (critical):**
    * Load `runs/detection_mapping_greedy.jsonl` and compute decision_step from
      the first occurrence of X.
    * If `word_present == false`, set decision_step = 0 (first generated token).
    * If `mapping_error == true`, skip the entry.
    * Measure attention at `attentions[decision_step]` (not always step 0).
